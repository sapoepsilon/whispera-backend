...
import { defaultRecipeModel } from '../../config/models.js';

/**
 * Built at call time (not module load) so DEFAULT_RECIPE_MODEL is read after
 * the env file has been loaded. Defaults to gpt-4o-mini when unset.
 */
export function buildDefaultRecipes(
  model: string = defaultRecipeModel(),
): ReadonlyArray<Omit<NewRecipe, 'userId'>> {
  return [
    {
      name: 'Polish speech',
      description:
...
export class RecipeSeedService {
  ...
  async seedDefaultsForUser(userId: string): Promise<{ created: number }> {
    ...
    const rows: NewRecipe[] = buildDefaultRecipes().map((r) => ({ ...r, userId }));
